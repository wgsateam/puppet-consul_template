...
=end
  ...
  dispatch :consul_template_sorted_json do
    optional_param 'Hash', :unsorted_hash
    optional_param 'Boolean', :pretty
    optional_param 'Integer', :indent_len
  end

  def consul_template_sorted_json(unsorted_hash={}, pretty=false, indent_len=4)
    return sorted_pretty_generate(unsorted_hash, indent_len) << "\n" if pretty
    sorted_generate(unsorted_hash)
  end
end
